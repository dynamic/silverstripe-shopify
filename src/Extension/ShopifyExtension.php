...
class ShopifyExtension extends Extension
{
    ...
    public function getCartOptions()
    {
        ...
        if ($limit = $this->owner->config()->get('noteLimit')) {
            $config['cart']['templates']['footer'] = '{{^data.isEmpty}}
            <div class="{{data.classes.cart.footer}}" data-element="cart.footer">
              ...
              {{#data.contents.note}}
                <div class="{{data.classes.cart.note}}" data-element="cart.note">
                  ...
                  <textarea id="{{data.cartNoteId}}" class="{{data.classes.cart.noteTextArea}}" data-element="cart.noteTextArea" rows="3" maxlength="' . $limit . '"/>{{data.cartNote}}</textarea>
                </div>
              {{/data.contents.note}}
    ...
    public function onAfterInit()
    {
        ...
        Requirements::javascript(
            'dynamic/silverstripe-shopify:client/cart.init.js',
            [
            "defer" => true,
            ]
        );
